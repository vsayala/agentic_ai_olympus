...
3. Add numbered source notebooks under `notebooks/<NN>_<source>/` and compose them into one job
   under `resources/jobs/` with explicit linear dependencies.
4. Put source logic in `src/olympus_databricks/sources/<source>` and cross-source logic in
   `src/olympus_databricks/utilities`; notebooks only adapt widgets, secrets, Spark, and SDK clients.
...
Use Databricks `.py` source notebooks unless rich saved outputs are required. The job environment
installs the project wheel; never commit or sync a local `.venv`.
